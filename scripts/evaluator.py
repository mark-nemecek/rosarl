...
class Evaluator:  # pylint: disable=too-many-instance-attributes
    ...
    def evaluate(
        ...
        for episode in range(num_episodes):
            obs, _ = self._env.reset(seed=episode)
            self._safety_obs = torch.ones(1)
            ep_ret, ep_cost, length, ep_success = 0.0, 0.0, 0.0, 0.0
            ...
            print(f'Episode success: {ep_success}')
        ...
        print(f'Average episode success: {np.mean(a=episode_successes)}')
